...
export default {
  ...
  setup() {
    ...
    const changeRoute = async (to) => {
      if (to.name === 'OperatorClientsFind') {
        clearData()
        return
      }

      if (to.name === 'OperatorClientsFindUpdate' || to.name === 'OperatorClientsFindJoin' || to.name === 'OperatorClientsFindJoinAddress') {
        loading.value = false
        const findClient = getClients.value.find(client => +client.id === +route.params.client)
        if (!findClient) {
          router.push({name: 'OperatorClientsUpdate', params: {client: route.params.client}})
        }
      ...
      if (to.name === 'OperatorClientsFindPreviewOrder') {
...
<template>
  <div class="clients-find">
    <u-button
        ...
        @click="router.push({name: 'OperatorClientsFindSetting'})"
    >
      Настройка поиска
    </u-button>
        ...
        <u-card class="clients-find__orders-list">
          ...
          >
            <orders-list
                ...
                :check-blank="false"
                @preview="e => router.push({name: 'OperatorClientsFindPreviewOrder', params: {id: e}})"
            />
          </u-accordion>
        </u-card>
        <u-actions
            ...
            @update="router.push({name: 'OperatorClientsFindUpdate', params: {client: clientItem.id}})"
            @joinClients="router.push({name: 'OperatorClientsFindJoin', params: {client: clientItem.id}})"
            @joinAddress="router.push({name: 'OperatorClientsFindJoinAddress', params: {client: clientItem.id}})"
        />
      </u-card>
      <u-button
          class="orders__find"
    ...
    <u-popup
        v-if="route.name === 'OperatorClientsFindSetting'"
        title="Настройка поиска"
        @close="router.push({name: 'OperatorClientsFind'})"
    >
      <u-form
          text="Найти клиентов"
          @submit.prevent="submitFindClientsList('OperatorClientsFind')"
      >
        <u-input
            title="ФИО"
    ...
    <u-popup
        v-if="loading && route.name === 'OperatorClientsFindUpdate' && route.params.client"
        title="Изменение клиента"
        @close="router.push({name: 'OperatorClientsFind'})"
    >
      <u-form
          text="Изменить клиента"
          @submit.prevent="submitUpdateClients('OperatorClientsFind')"
      >
        <u-input
            title="ФИО"
            v-model="name.value"
    ...
    <u-popup
        v-if="loading && route.name === 'OperatorClientsFindJoin' && route.params.client"
        title="Объединение клиентов"
        @close="router.push({name: 'OperatorClientsFind'})"
    >
      <u-form
          text="Объдинить клиентов"
          @submit.prevent="submitJoinClients('OperatorClientsFind')"
      >
        <p class="clients-find__text">
          <b>ФИО: </b>{{ name.value }}
        </p>
    ...
    <u-popup
        v-if="loading && route.name === 'OperatorClientsFindJoinAddress' && route.params.client"
    ...
    <orders-preview
        v-if="route.name === 'OperatorClientsFindPreviewOrder' && getGoodsList.length && getProductsList.length && getKitsList.length && getPresentsList.length && getOrderDetail"
