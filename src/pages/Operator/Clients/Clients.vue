...
export default {
  ...
  setup() {
    ...
    const changeRoute = async (to) => {
      if (to.name === 'OperatorClients') {
        clearData()
        return
      }

      if (to.name === 'OperatorClientsUpdate' || to.name === 'OperatorClientsJoin' || to.name === 'OperatorClientsJoinAddress') {
        loading.value = false
      ...
      if (to.name === 'OperatorClientsPreviewOrder') {
...
<template>
  <div class="clients">
        ...
        <u-card class="clients__orders-list">
          ...
          >
            <orders-list
                ...
                :check-blank="false"
                @preview="e => router.push({name: 'OperatorClientsPreviewOrder', params: {id: e}})"
            />
          </u-accordion>
        </u-card>
        <u-actions
            ...
            @update="router.push({name: 'OperatorClientsUpdate', params: {client: clientItem.id}})"
            @joinClients="router.push({name: 'OperatorClientsJoin', params: {client: clientItem.id}})"
            @joinAddress="router.push({name: 'OperatorClientsJoinAddress', params: {client: clientItem.id}})"
        />
      </u-card>
      <u-button
          class="orders__find"
    ...
    <u-popup
        v-if="loading && route.name === 'OperatorClientsUpdate' && route.params.client"
        title="Изменение клиента"
        @close="router.push({name: 'OperatorClients'})"
    >
      <u-form
          text="Изменить клиента"
          @submit.prevent="submitUpdateClients('OperatorClients')"
      >
        <u-input
            title="ФИО"
            v-model="name.value"
    ...
    <u-popup
        v-if="loading && route.name === 'OperatorClientsJoin' && route.params.client"
        title="Объединение клиентов"
        @close="router.push({name: 'OperatorClients'})"
    >
      <u-form
          text="Объдинить клиентов"
          @submit.prevent="submitJoinClients('OperatorClients')"
      >
        <p class="clients__text">
          <b>ФИО: </b>{{ name.value }}
        </p>
    ...
    <u-popup
        v-if="loading && route.name === 'OperatorClientsJoinAddress' && route.params.client"
        title="Объединение адресов клиента"
        @close="router.push({name: 'OperatorClients'})"
    >
      <u-form
          text="Объдинить адреса"
          @submit.prevent="submitJoinAddress('OperatorClients')"
      >
        <p class="clients__text">
          <b>ФИО: </b>{{ name.value }}
        </p>
    ...
    <orders-preview
        v-if="route.name === 'OperatorClientsPreviewOrder' && getGoodsList.length && getProductsList.length && getKitsList.length && getPresentsList.length && getOrderDetail"
